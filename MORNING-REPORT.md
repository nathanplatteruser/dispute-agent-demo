...
- **Offline verification not done with network actually off.** The brief says to verify by turning wifi off. I tested Tier 2 (Ollama) which works offline, and Tier 3 (cache) which is fully local. But I did not turn off the network adapter during testing. Do this before Aug 24.
...
**Run through the RUN-OF-SHOW.md once, start to finish, with a stopwatch.** That's the gap between MVP and rehearsal-ready. Time each section, mark which ones run long, and adjust. The pipeline is built. The talk needs your voice on it.
