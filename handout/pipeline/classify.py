...
-------------------------------------
Every dispute needs to be categorized before the pipeline can draft a
response. This stage reads the consumer's complaint and decides which
type of dispute it is:

  - identity_theft:          Someone else opened the account
  - not_mine:                Consumer says the debt isn't theirs
  - already_paid:            Consumer says they already paid
  - wrong_amount:            Consumer disputes the balance
  - validation_request:      Consumer wants proof the debt is valid
  - statute_of_limitations:  Debt is too old to collect
  - hardship:                Consumer can't pay, wants an arrangement

It tries three approaches in order:

  1. KEYWORD MATCHING (fast, no AI needed): Looks for phrases like
     "identity theft," "already paid," "statute of limitations" in the
     consumer's narrative. If the match is strong, classification is done.

  2. LEDGER DATA: If the synthetic ledger already has a dispute type
     assigned, use that.

  3. LLM (AI model): For ambiguous cases, asks the language model to read
     the narrative and pick the best category. This is the slowest path
     but handles edge cases the keywords miss.

Most records get classified by keywords alone. The LLM is a fallback,
not the default.
"""
